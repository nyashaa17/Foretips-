...
export default function SignUp() {
      ...
      if (signUpError) {
        ...
        if (data?.user) {
          const { error: profileError } = await supabase
            .from('profiles')
            .upsert([{ id: data.user.id, username: username || email.split('@')[0] }]);
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16">
      ...
      {message && <div className="bg-blue-50 text-blue-700 p-4 rounded-lg mb-6 text-center font-bold">{message}</div>}
      ...
      <div className="flex flex-col items-center justify-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-slate-700 text-center">Create an Account</h1>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <p className="mt-4 text-center text-sm text-slate-600">
          Already have an account? <Link to="/signin" className="text-green-600 font-bold">Sign In</Link>
        </p>
        </form>
      </div>
    </div>
